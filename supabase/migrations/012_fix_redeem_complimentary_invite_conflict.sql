-- =============================================================================
-- Fix redeem complimentary invite ON CONFLICT ambiguity (additive)
-- =============================================================================
...
-- 010_tenant_complimentary_access_invites.sql, and
-- 011_redeem_tenant_complimentary_access_invite.sql.
--
-- CREATE OR REPLACE of:
--   public.redeem_tenant_complimentary_access_invite(text)
--
-- Public contract is unchanged (signature, RETURNS TABLE, SECURITY INVOKER,
-- search_path, grants, domain reasons, lock/expiry/revoke/redeem checks,
-- grant 009 insert-or-update, consume invite 010).
--
-- Sole correction: ON CONFLICT target. RETURNS TABLE exposes an OUT column
-- named tenant_id; PL/pgSQL treated ON CONFLICT (tenant_id) as that OUT
-- parameter instead of the grants PK column, raising at runtime.
--
-- Constraint name from 009: implicit PRIMARY KEY on tenant_id is
-- tenant_complimentary_access_grants_pkey.
-- =============================================================================

create or replace function public.redeem_tenant_complimentary_access_invite(p_token_hash text)
returns table (
  ok boolean,
  reason text,
  invite_id uuid,
...
begin
  ...
  on conflict on constraint tenant_complimentary_access_grants_pkey
    do update set product_tier = excluded.product_tier;
