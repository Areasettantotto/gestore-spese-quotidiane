...
|------|--------|
| Branch atteso | `main` |
| Commit applicativo di riferimento | `e87462cb3589cc69ca2ea712bd3c32564efe5f18` — *feat(billing): compose Stripe subscription refetch and normalization* — I4.3BJ |
| Ultimo commit governance/documentale consolidato | `cc36c06e791add2990cae676d46a4485b5abd329` — *docs(context): record subscription observation reader* — I4.3BI-bis |
| Verifica Git richiesta | All’inizio di ogni task: `git rev-parse HEAD`, `git status`, `git branch --show-current` |

Nota: l’HEAD reale deve essere verificato all’inizio di ogni task con `git rev-parse HEAD`. Il commit che aggiorna `stato-operativo.md` può essere successivo all’ultimo commit registrato nel documento; questo non costituisce automaticamente una divergenza. Branch, working tree e cronologia reale del repository prevalgono sempre sui valori storici riportati nel documento. Non riportare come HEAD stabile il futuro commit del task documentale corrente (I4.3BJ-bis).
...
- I4.3BF: **CHIUSO** — resolver tenant read-only fail-closed in `supabase/functions/_shared/resolveBillingCustomerTenant.ts` (+ test). Commit `48782cd` — *feat(billing): add billing customer tenant resolver*. Contratto: `provider` + `provider_customer_id` → SELECT server-side su `public.tenant_billing_customers` → `tenant_id`. Trust boundary: `tenant_id` solo da mapping server-side; **nessun** fallback da subscription/event/customer metadata, frontend o payload provider come ownership. SELECT-only; provider-aware; nessun Stripe API / network proprio / Deno.env / write / webhook wiring / W_sub / persistence / Snapshot / H2 / `processed_at` completion. Fail-closed: mapping unica → success; zero → `tenant_mapping_not_found`; più mapping → `tenant_mapping_ambiguous` (mai prima riga); query/throw → `tenant_mapping_lookup_failed`; mapping invalida → `tenant_mapping_invalid`; input invalidi → `invalid_provider` / `invalid_provider_customer_id`. Identità esatta degli input (nessuna normalizzazione lower-case/trim silenziosa). Quality: `npm run lint` PASS; `npm run build` PASS; `deno check` PASS; `deno test` 11/11 PASS; 0 FAIL; review Supervisor PASS (whitespace via `git diff --no-index --check` sui due file); nessun I4.3BF-F1. Rischio minore non bloccante: `BillingCustomerTenantLookupClient` strutturale non ancora compilato contro client Supabase reale (modulo non wired).
- I4.3BF-bis: consolidato in `a16eeb8` — *docs(context): record billing customer tenant resolver*.
- I4.3BF-D1: sync mirror controllata **completata e verificata** (preflight conforme; dry-run PASS; apply PASS; 15 file; byte/hash 15/15 PASS; cloud Drive verificato dal Supervisor; nessun `--delete`; nessuna sync inversa; repository invariato). La precedente formulazione storica «I4.3BF non pushato» / «sync I4.3BF-bis non eseguita» è quindi **risolta/verificata** (non riconciliazione silenziosa).
- Push I4.3BF + I4.3BF-bis: completato dall’utente (`613f3ed..a16eeb8` main → main); prima di I4.3BG, `main`/`origin/main` allineati a `a16eeb8` (verifica utente).
- I4.3BG: **CHIUSO** — primitive K2 provider-authoritative `refetchStripeSubscription` in `supabase/functions/_shared/refetchStripeSubscription.ts` (+ test). Commit `dc5cbcf` — *feat(billing): add Stripe subscription refetch primitive*. Contratto: `provider_subscription_id` → Stripe `subscriptions.retrieve(...)` → raw provider subscription result. Trust boundary: stato Subscription provider-authoritative solo da nuova retrieve via dependency server-side; **nessun** fallback da `event.data.object`, subscription object webhook, `billing_events.payload`, metadata, snapshot locale, cache/memoization. Ogni invocazione = retrieve indipendente. Dependency injection strutturale `StripeSubscriptionRetrieveClient` (`subscriptions.retrieve(id)`); helper senza import Stripe SDK, senza costruire client, senza Deno.env/secret, senza rete propria oltre alla dependency, senza DB/Supabase/tenant resolver/normalizer/W_sub/Snapshot/H2/`billing_events`/webhook wiring. Fail-closed: `invalid_provider_subscription_id` / `stripe_subscription_refetch_failed` / `stripe_subscription_refetch_invalid`; raw provider error non esposto. Input exact identity (stringa non vuota, non whitespace-only; nessun lower-case/trim sull’identità passata a retrieve). null/undefined response → `stripe_subscription_refetch_invalid`. Validazione semantica Subscription resta di `normalizeStripeSubscription`. **Non** implementa ancora CAS K2. Quality: `npm run lint` PASS; `npm run build` PASS; `deno check` PASS; `deno test` 12/12 PASS; 0 FAIL; nessuna capability `--allow-net`/`--allow-env`/`--allow-write`/`--allow-run`; review Supervisor PASS (whitespace via `git diff --no-index --check` sui due file untracked); nessun I4.3BG-F1. Rischio minore non bloccante: compatibilità strutturale del vero client Stripe non ancora provata dal wiring reale (verificare compatibilità diretta senza cast artificiali).
- I4.3BG-bis: consolidato in `bb0f775` — *docs(context): record Stripe subscription refetch primitive*.
- I4.3BG-D1: sync mirror controllata **completata e verificata** (preflight conforme; dry-run PASS; apply PASS; 15 file; byte/hash 15/15 PASS; cloud Drive verificato dal Supervisor; nessun `--delete`; nessuna sync inversa; repository invariato). La precedente formulazione storica «I4.3BG non pushato» / «sync I4.3BG-bis non eseguita» è quindi **risolta/verificata** (non riconciliazione silenziosa).
- Push I4.3BG + I4.3BG-bis: completato dall’utente (`a16eeb8..bb0f775` main → main); prima di I4.3BH, `main`/`origin/main` allineati a `bb0f775` (verifica utente).
- I4.3BH: **CHIUSO** — classificatore puro/deterministico admission W_sub `classifySubscriptionEventAdmission` in `supabase/functions/_shared/classifySubscriptionEventAdmission.ts` (+ test). Commit `38e4280` — *feat(billing): add subscription event admission classifier*. Contratto: input `provider_event_created_at` + `provider_event_id` + `billing_event_processed` + osservazione `tenant_subscriptions` (`ROW_ABSENT` oppure `ROW_PRESENT` con W_sub); **non** esegue il ramo classificato. Distinzione obbligatoria: `ROW_ABSENT` ≠ `ROW_PRESENT` con W NULL/NULL (INSERT vs conditional UPDATE/CAS). Classificazioni positive: `candidate_row_absent` / `candidate_row_present_uninitialized` / `candidate_newer_event` / `candidate_equal_timestamp_distinct_event` / `stale_event` / `partial_retry` / `already_applied`. Failure: `invalid_provider_event_created_at` / `invalid_provider_event_id` / `invalid_watermark` / `inconsistent_same_event`. `provider_event_id` = exact identity (**non** trim/lower-case; **non** clock; **non** chronological/lessicographic tie-break). Equal timestamp + ID distinti → `candidate_equal_timestamp_distinct_event`. Half-null W → `invalid_watermark` (authority semantica del classifier; I4.3BI preserva half-null type-valid senza duplicare questa semantica). Same Event + `billing_event_processed=false` → `partial_retry` (guardrail I4.3BB-R-F1: **non** completed); same Event + processed → `already_applied`; same Event ID + timestamp diverso → `inconsistent_same_event`. Puro: zero DB/Supabase/Stripe SDK/provider I/O/Deno.env/fetch/Date.now/wall-clock/cache/memoization/webhook wiring/persistence/CAS/Snapshot/H2. Quality: `npm run lint` PASS; `npm run build` PASS; warning Vite chunk size preesistente/non bloccante; `deno check` PASS; `deno test` 18/18 PASS; 0 FAIL; nessuna capability `--allow-net`/`--allow-env`/`--allow-write`/`--allow-run`; review Supervisor PASS (whitespace via `git diff --no-index --check` sui due file untracked); nessun I4.3BH-F1. Rischio minore non bloccante: `billing_event_processed` tipizzato boolean non validato runtime nel classifier — il futuro wiring deve derivarlo esplicitamente da `processed_at` NULL/NOT NULL.
- I4.3BH-bis: consolidato in `57c7e00` — *docs(context): record subscription event admission classifier*.
- I4.3BH-D1: sync mirror controllata **completata e verificata** (preflight conforme; dry-run PASS; apply PASS; 15 file; byte/hash 15/15 PASS; cloud Drive verificato dal Supervisor; nessun `--delete`; nessuna sync inversa; repository invariato). La precedente formulazione storica «I4.3BH non pushato» / «sync I4.3BH-bis non eseguita» (e snapshot Drive/header che ancora citavano I4.3BG-bis come ultimo governance) è quindi **risolta/verificata** (non riconciliazione silenziosa).
- Push I4.3BH + I4.3BH-bis: completato dall’utente (`bb0f775..57c7e00` main → main); prima di I4.3BI, `main`/`origin/main` allineati a `57c7e00` (verifica utente).
- I4.3BI: **CHIUSO** — reader server-side READ-ONLY `readTenantSubscriptionObservation` in `supabase/functions/_shared/readTenantSubscriptionObservation.ts` (+ test). Commit `92fb6dd` — *feat(billing): add subscription observation reader*. Contratto: `provider` + `provider_subscription_id` + client strutturale minimale → SELECT-only su `tenant_subscriptions` → `ROW_ABSENT` (zero righe) oppure `ROW_PRESENT` (`tenant_id` + W_sub). Filtri exact identity `provider` / `provider_subscription_id` (stringhe non vuote; whitespace-only rifiutato; nessun trim/lower-case sull’identità query; padded valid identity passata esattamente). Campi letti: `tenant_id`, `last_applied_provider_event_created_at`, `last_applied_provider_event_id`. **ROW_ABSENT** deriva esclusivamente da zero righe DB; **ROW_PRESENT** con W NULL/NULL resta present (**non** collassare present NULL/NULL in absent). Half-null strategy **A — preservation**: reader preserva half-null type-valid senza interpretazione semantica; `invalid_watermark` resta di `classifySubscriptionEventAdmission` (I4.3BH). `tenant_id` osservato (stringa non vuota; exact identity; nessun remap); ownership finale **non** decisa — futuro orchestrator confronterà con tenant risolto da I4.3BF (mismatch fail-closed). Cardinalità array-based: 0→ABSENT; 1→PRESENT; >1→`subscription_observation_ambiguous` fail-closed (mai prima riga). W_sub validation: created null o number finite integer safe ≥0 (no string/Date/wall-clock); event id null o stringa non vuota non whitespace-only exact. Failures: `invalid_provider` / `invalid_provider_subscription_id` / `subscription_observation_lookup_failed` / `subscription_observation_ambiguous` / `subscription_observation_invalid`. Error sanitization: query/throw → lookup_failed senza raw DB/message/code/token/path. Read-only boundary: SELECT-only; zero INSERT/UPDATE/UPSERT/DELETE/RPC/raw SQL write; zero Stripe/provider I/O; zero Deno.env; zero wall-clock; zero cache; zero classifier call; zero resolver orchestration; zero webhook wiring; zero persistence/CAS; zero Snapshot/H2; zero `processed_at` completion. Client strutturale minimale `TenantSubscriptionObservationLookupClient` (from→select→eq→eq→PromiseLike); nessun cast artificioso; nessun service_role; nessun client Supabase creato nel modulo. I4.3BI-F1: **CHIUSO** — hygiene esclusivamente test (marker sintetici credential/token/path-like sostituiti da sentinel neutri `RAW_*`; reader byte-identico SHA-256 `7bed5e5ea86c8ea184fd889a5f91aedeb2657515166735701d93763601d36614`; hygiene grep PASS; nessun F2). Quality: `npm run lint` PASS; `npm run build` PASS; warning Vite chunk size preesistente/non bloccante; `deno check` PASS; `deno test` 20/20 PASS; 0 FAIL; nessuna capability `--allow-net`/`--allow-env`/`--allow-write`/`--allow-run`; whitespace `git diff --no-index --check` PASS su entrambi i nuovi file; review Supervisor PASS. Rischio minore non bloccante: compatibilità strutturale diretta col client Supabase server-side reale non ancora provata dal wiring. Helper **NON WIRED**.
- I4.3BI-bis: consolidato in `cc36c06` — *docs(context): record subscription observation reader*.
- I4.3BI-D1: sync mirror controllata **completata e verificata** (preflight conforme; dry-run PASS; apply PASS; 15 file; byte/hash 15/15 PASS; cloud Drive verificato dal Supervisor; nessun `--delete`; nessuna sync inversa; repository invariato). La precedente formulazione storica «I4.3BI non pushato» / «sync I4.3BI-bis non eseguita» (e snapshot/header che ancora citavano I4.3BH-bis come ultimo governance) è quindi **risolta/verificata** (non riconciliazione silenziosa).
- Push I4.3BI + I4.3BI-bis: completato dall’utente (`57c7e00..cc36c06` main → main); prima di I4.3BJ, `main`/`origin/main` allineati a `cc36c06` (verifica utente).
- I4.3BJ: **CHIUSO APPLICATIVAMENTE / COMMITTATO LOCALMENTE** — composer provider-authoritative `fetchNormalizedStripeSubscription` in `supabase/functions/_shared/fetchNormalizedStripeSubscription.ts` (+ test). Commit `e87462c` — *feat(billing): compose Stripe subscription refetch and normalization*. Compone direttamente `refetchStripeSubscription` (I4.3BG) seguito da `normalizeStripeSubscription` (I4.3BE). Sequenza non negoziabile: `provider_subscription_id` → fresh Stripe retrieve via BG → normalize via BE → normalized subscription. Su failure BG: STOP; `stage=refetch`; reason originaria preservata; BE **non** eseguito nel success path. Su failure BE: STOP; `stage=normalize`; reason originaria preservata. Su success: restituisce il normalized result di BE. Trust boundary: nessun fallback da `event.data.object` / `billing_events.payload` / snapshot locale/cache / payload webhook; nessuna duplicate normalization; nessuna seconda business interpretation dei campi Stripe; `provider_subscription_id` affidato semanticamente a BG (nessuna nuova canonicalizzazione nel composer); normalizer config esplicita inoltrata a BE; nessun Deno.env nel runtime del composer; nessun DB/Supabase client; nessuna tenant resolution / ownership check / row observation / classifier invocation; nessun W_sub / persistence / INSERT/UPDATE/CAS / 23505 / Snapshot(S) / H2 / `processed_at` completion / `billing_events` mutation / webhook wiring / deploy / apply remoto migration 007. Boundary TypeScript: BG success `subscription: unknown` → cast locale esplicito `unknown` → `StripeSubscriptionLike` → BE; cast accettato dalla review Supervisor perché non rimappa il raw provider result, non duplica validazione (semantica resta di BE), non finge compatibilità del vero SDK Stripe. Descrizione corretta: composizione runtime interna BG→BE verificata; boundary TypeScript `unknown`→`StripeSubscriptionLike` esplicito; compatibilità del vero client Stripe SDK resta **non** verificata fino al wiring reale. I4.3BJ-F1: **CHIUSO** — esclusivamente test-only; composer applicativo byte-identico SHA-256 `ce252fca09b50076ae17c5a18b40bb3f6e9d527a982258b9d590e9bed03a13ea`; nessun F2. F1 ha corretto: (1) repository hygiene — rimossi marker sintetici credential/raw-detail-like problematici; hygiene check PASS; (2) test config — rimossa assertion sempre-vera; config esplicita corretta → success; config differente → normalization failure `unsupported_price` (dimostra forwarding config); (3) test no-fallback — rimosso fake webhook/event object inutilizzato; provider retrieve failure → `stage=refetch`; contratto pubblico failure limitato a `ok`/`stage`/`reason`; nessun value da fonte alternativa. Quality: `npm run lint` PASS; `npm run build` PASS; warning Vite chunk size preesistente/non bloccante; `deno check fetchNormalizedStripeSubscription.ts` PASS; I4.3BJ tests 12/12 PASS; 0 FAIL; regressione I4.3BG 12/12 PASS; regressione I4.3BE 28/28 PASS; nessuna capability `--allow-net`/`--allow-env`/`--allow-write`/`--allow-run`; whitespace PASS; review Supervisor PASS dopo I4.3BJ-F1. Rischio residuo: `StripeSubscriptionRetrieveClient` di BG non ancora verificato contro il vero client Stripe nel wiring reale — I4.3BJ dimostra composizione interna BG+BE, **non** prova che il vero Stripe SDK/client runtime soddisfi direttamente il structural contract senza adattamenti. Composer **NON WIRED**. **Non** pushato su `origin/main` in questo snapshot.
- Governance: GOVERNANCE-5-bis in 063cbf7; GOVERNANCE-6-bis in 253affa; I4.3A-bis in 99dc1f6; GOVERNANCE-7 consolidato in 8ff556d; GOVERNANCE-7-bis consolidato in a380ce9; I4.3A-D1-bis consolidato in 751852b; I4.3A-T1-bis consolidato in b4da681; I4.3A-T1C-bis consolidato in 2c0c060; I4.3A-T1D-bis consolidato in 87d410c; I4.3A-T1E-bis consolidato in 8615cda; I4.3A-T1F-bis consolidato in d4a2889; I4.3A-T1G-bis consolidato in 30da6a5; I4.3BA-bis consolidato in 436d587; I4.3BB-bis consolidato in 169dfa4; I4.3BC-bis consolidato in 83fbf24; I4.3BD-bis consolidato in aa2b1e7; I4.3BE-bis consolidato in 613f3ed; I4.3BF-bis consolidato in a16eeb8; I4.3BG-bis consolidato in bb0f775; I4.3BH-bis consolidato in 57c7e00; I4.3BI-bis consolidato in cc36c06.
- Mirror: Git canonico, Drive = consultazione. Sync post-commit I4.3BD-bis (`aa2b1e7`) **completata e verificata**. Sync post-commit I4.3BE-bis (`613f3ed`) / I4.3BE-D1 **completata e verificata** (dry-run PASS; apply PASS; byte/hash 15/15 PASS; cloud verificato dal Supervisor; nessuna sync inversa; nessun `--delete`). Sync post-commit I4.3BF-bis (`a16eeb8`) / I4.3BF-D1 **completata e verificata** (dry-run PASS; apply PASS; 15 file; byte/hash 15/15 PASS; cloud verificato dal Supervisor; nessun `--delete`; nessuna sync inversa; repo invariato). Sync post-commit I4.3BG-bis (`bb0f775`) / I4.3BG-D1 **completata e verificata** (dry-run PASS; apply PASS; 15 file; byte/hash 15/15 PASS; cloud verificato dal Supervisor; nessun `--delete`; nessuna sync inversa; repo invariato). Sync post-commit I4.3BH-bis (`57c7e00`) / I4.3BH-D1 **completata e verificata** (dry-run PASS; apply PASS; 15 file; byte/hash 15/15 PASS; cloud verificato dal Supervisor; nessun `--delete`; nessuna sync inversa; repo invariato). Sync post-commit I4.3BI-bis (`cc36c06`) / I4.3BI-D1 **completata e verificata** (dry-run PASS; apply PASS; 15 file; byte/hash 15/15 PASS; cloud verificato dal Supervisor; nessun `--delete`; nessuna sync inversa; repo invariato). Sync post-commit I4.3BJ-bis **non ancora eseguita** (obbligatoria dopo il futuro commit di questo consolidamento; non inventare hash). Commit/push restano dell’utente. Config locale non versionata (AI_CONTEXT_MIRROR_DIR o .git/ai-context-mirror-path). Prima sync controllata completata su 15 file. Propagazione cloud = Google Drive Desktop. ChatGPT rilegge il mirror a ogni ripresa.
- ANCORA MANCANTE: sync subscription → tenant_subscriptions + snapshot tenants (I4.3B, **NON COMPLETO** complessivamente; schema M2/W_sub/revision/G1 **implementati** in I4.3BD e testati in locale; normalizzatore provider puro **implementato** in I4.3BE; tenant resolution helper read-only **implementato** in I4.3BF; provider re-fetch primitive **implementato** in I4.3BG; pure admission classifier W_sub **implementato** in I4.3BH; mapping/read helper row observation `tenant_subscriptions` **implementato** in I4.3BI come helper read-only **NON WIRED**; composer provider-authoritative refetch→normalize **implementato** in I4.3BJ come helper/composer **NON WIRED**; restano assenti runtime wiring/orchestration del composer I4.3BJ / runtime wiring del resolver BF / runtime wiring del reader BI / ownership check row `tenant_id` vs tenant risolto / classifier invocation runtime / admission branch execution / persistence `tenant_subscriptions` (INSERT/conditional UPDATE/CAS) / 23505 relookup / CAS failure readback / nuovo provider re-fetch dopo CAS failure / bounded retry / partial retry downstream / Snapshot(S) V3 / H2 CAS V2 / TenantGuard / webhook wiring `customer.subscription.*` / valorizzazione `billing_events.tenant_id` nel path subscription / `processed_at` completion / runtime-deploy / apply remoto migration 007; policy prodotto D3/D4/D5/D7/unpaid **congelate** in I4.3BC); invoice fuori scope I4.3B; billing portal; UI checkout reale; feature gating piani; tenant switcher/inviti. Drift storico di production-readiness.md e billing-data-model.md ancora presente.

Filone corrente: I4.3B — subscription sync e tenant snapshot (**NON COMPLETO** complessivamente). Schema M2 completato in I4.3BD; normalizzatore puro completato in I4.3BE; resolver tenant read-only completato in I4.3BF; provider re-fetch primitive completato in I4.3BG; pure admission classifier W_sub completato in I4.3BH; row observation reader read-only completato in I4.3BI (helper **NON WIRED**); composer refetch→normalize completato in I4.3BJ (helper **NON WIRED**); restano runtime wiring/orchestration del composer / runtime wiring del resolver BF / runtime wiring del reader BI / ownership check / classifier runtime invocation / admission branch execution / persistence/CAS / Snapshot H2 / wiring eventi subscription / runtime/deploy remoto. Il prossimo slice I4.3B sarà deciso dal Supervisor dopo review di questo documentale + commit I4.3BJ-bis + sync mirror + verifica cloud + push Git e verifica allineamento; **NON** avviare autonomamente ownership/persistence/CAS/wiring/Snapshot/H2/deploy. I4.3A-T1 = CHIUSO COMPLESSIVAMENTE. I4.3A-D1 concluso. I4.3BA / I4.3BB / I4.3BB-R / F1–F3 / I4.3BC / F1–F3 / I4.3BD / I4.3BD-T1 / I4.3BD-bis / I4.3BE / I4.3BE-F1 / I4.3BE-T1 / I4.3BE-bis / I4.3BE-D1 / I4.3BF / I4.3BF-bis / I4.3BF-D1 / I4.3BG / I4.3BG-bis / I4.3BG-D1 / I4.3BH / I4.3BH-bis / I4.3BH-D1 / I4.3BI / I4.3BI-F1 / I4.3BI-bis / I4.3BI-D1 / I4.3BJ / I4.3BJ-F1 chiusi (I4.3BJ ancora non pushato; K2 complessivo ancora **NON** implementato).
...
| **I4.3BJ / I4.3BJ-F1** | **CHIUSO APPLICATIVAMENTE / COMMITTATO LOCALMENTE** — composer provider-authoritative refetch→normalize (`e87462c`); F1 test-only chiuso; 12/12 Deno PASS; regressione BG 12/12 + BE 28/28; non ancora pushato; consolidamento documentale = questo task I4.3BJ-bis (sync mirror **non** ancora eseguita). Helper **NON WIRED**. K2 complessivo ancora **NON** implementato |
| **I4.3B (filone corrente)** | Sync `customer.subscription.*` → `tenant_subscriptions` + snapshot `tenants` — **NON COMPLETO** complessivamente. Schema M2 / W_sub / `billing_state_revision` / trigger G1 = **completati** in I4.3BD. Normalizzatore provider puro = **completato** in I4.3BE. Tenant resolution helper read-only = **completato** in I4.3BF. Provider re-fetch primitive = **completato** in I4.3BG. Pure admission classifier W_sub = **completato** in I4.3BH. Mapping/read helper row observation = **completato** in I4.3BI (read-only **NON WIRED**). Composer provider-authoritative refetch→normalize = **completato** in I4.3BJ (helper **NON WIRED**). Restano **non** implementati: runtime wiring/orchestration del composer I4.3BJ; runtime wiring del resolver BF; runtime wiring del reader BI; ownership check (`tenant_subscriptions.tenant_id` vs tenant I4.3BF); mapping `processed_at` NULL/NOT NULL → `billing_event_processed`; classifier invocation runtime; admission branch execution; persistence `tenant_subscriptions` (INSERT / conditional UPDATE/CAS / 23505 relookup / CAS failure readback); nuovo provider re-fetch dopo CAS failure se Event resta candidato; bounded retry; partial_retry downstream; Snapshot(S) V3 applicativa; H2 CAS V2 / TenantGuard; wiring `customer.subscription.created|updated|deleted`; valorizzazione `billing_events.tenant_id` nel path subscription; `processed_at` completion; runtime/deploy; apply remoto migration 007. Policy Snapshot V3 **congelate** ma **non** coded nel runtime. Invoice fuori scope del primo I4.3B. Eventi `customer.subscription.*` restano differiti (`processed_at` nullo) finché un task applicativo successivo non implementa esplicitamente i slice residui |
| **Sync subscription** | Eventi `customer.subscription.*` / invoice: **solo persistiti** in `billing_events` con `processed_at` nullo; **non** risolvono `tenant_id` a runtime; **non** eseguono handler business; **non** aggiornano `tenant_subscriptions` né lo snapshot su `tenants`; rispondono HTTP 200 deferred. Architettura anti-stale/snapshot + policy prodotto progettate (K2+R-A+Snapshot V3+H2 V2); schema M2 presente in locale; normalizzatore puro presente; resolver helper read-only presente; provider re-fetch primitive presente; pure admission classifier presente; row observation reader presente (**NON WIRED**); composer refetch→normalize presente (**NON WIRED**); **non** wired a runtime |
...
| **Debito operativo `processed_at=NULL`** | Oggi significa sia deferred intenzionale sia evento incompleto — da affrontare separatamente; non in I4.3BJ-bis |
...
72. **I4.3BJ** — provider-authoritative refetch→normalize composer (`fetchNormalizedStripeSubscription.ts` + test), consolidato in `e87462c` — *feat(billing): compose Stripe subscription refetch and normalization*; BG→BE; fail-closed per stage; zero fallback/webhook/DB/wiring; boundary `unknown`→`StripeSubscriptionLike`; non ancora pushato
73. **I4.3BJ-F1** — test-only (hygiene + config forwarding + no-fallback); composer SHA-256 invariato; **CHIUSO**; nessun F2
74. **I4.3BJ-bis** — consolidamento documentale composition refetch→normalize (questo task; commit da registrare al prossimo aggiornamento significativo; sync mirror obbligatoria dopo il futuro commit)
...
**I4.3BJ:** **CHIUSO APPLICATIVAMENTE / COMMITTATO LOCALMENTE** (non ancora pushato). Commit `e87462c` — *feat(billing): compose Stripe subscription refetch and normalization*. Composer provider-authoritative: `refetchStripeSubscription` (I4.3BG) → `normalizeStripeSubscription` (I4.3BE). Sequenza non negoziabile; fail-closed per stage (`refetch` / `normalize`) con reason originaria preservata; su failure BG, BE non eseguito. Nessun fallback webhook/payload/snapshot/cache; nessuna duplicate normalization; config esplicita inoltrata a BE; nessun Deno.env/DB/Supabase/tenant/ownership/observation/classifier/W_sub/persistence/CAS/Snapshot/H2/wiring/deploy. Boundary TypeScript: BG `subscription: unknown` → cast locale `unknown`→`StripeSubscriptionLike` → BE (accettato: non rimappa; validazione semantica resta di BE; non prova compatibilità SDK). Descrizione corretta: composizione runtime interna BG→BE verificata; boundary TypeScript esplicito; compatibilità del vero client Stripe SDK resta non verificata fino al wiring reale. I4.3BJ-F1 **CHIUSO** (test-only; composer SHA-256 invariato `ce252fca09b50076ae17c5a18b40bb3f6e9d527a982258b9d590e9bed03a13ea`; hygiene PASS; config forwarding reale; no-fallback; nessun F2). Quality: `npm run lint` PASS; `npm run build` PASS; warning Vite chunk size preesistente/non bloccante; `deno check` PASS; `deno test` 12/12 PASS; regressione BG 12/12; regressione BE 28/28; review Supervisor PASS dopo F1. Allo stato Git locale di preflight I4.3BJ-bis: `main` ahead di `origin/main` di 1 — **non** attestare push di I4.3BJ. Composer **NON WIRED**. K2 complessivo = ancora **NON** implementato (composer helper ≠ runtime orchestration/persistence/CAS/wiring).
...
**I4.3B:** **NON COMPLETO** complessivamente. Schema M2 / W_sub / `billing_state_revision` / G1 = **completati** in I4.3BD. Normalizzatore provider puro = **completato** in I4.3BE. Tenant resolution helper read-only = **completato** in I4.3BF. Provider re-fetch primitive = **completato** in I4.3BG. Pure admission classifier W_sub = **completato** in I4.3BH. Mapping/read helper row observation = **completato** in I4.3BI (read-only **NON WIRED**). Composer provider-authoritative refetch→normalize = **completato** in I4.3BJ (helper **NON WIRED**). Restano **non** implementati: runtime wiring/orchestration del composer I4.3BJ; runtime wiring del resolver BF; runtime wiring del reader BI; ownership check row `tenant_id` vs tenant risolto I4.3BF; mapping `processed_at` NULL/NOT NULL → `billing_event_processed`; classifier invocation runtime; admission branch execution; persistence `tenant_subscriptions` (INSERT/conditional UPDATE/CAS/23505/CAS failure readback); nuovo provider re-fetch dopo CAS failure se l’Event resta candidato; bounded retry; partial_retry downstream; Snapshot(S) V3 applicativa; H2 CAS V2 / TenantGuard; wiring `customer.subscription.created|updated|deleted`; valorizzazione `billing_events.tenant_id` nel path subscription; `processed_at` completion; runtime/deploy; apply remoto migration 007. Invoice fuori scope del primo I4.3B. Eventi `customer.subscription.*` restano differiti finché un task applicativo successivo non implementa esplicitamente i slice residui. K2 congelato resta: provider re-fetch + admission watermark W_sub + CAS/readback + nuovo provider re-fetch dopo CAS failure se l’Event resta candidato + retry locale limitato + fail/retry se contention non converge — I4.3BG implementa il primitive refetch; I4.3BH implementa **soltanto** il classificatore puro di admission; I4.3BI implementa **soltanto** il reader observation read-only; I4.3BJ implementa **soltanto** il composer refetch→normalize; runtime wiring / persistence / CAS / orchestration restano assenti.

**Filone corrente: I4.3B** — subscription sync e tenant snapshot (**NON COMPLETO**). Schema M2 completato in I4.3BD; normalizzatore puro completato in I4.3BE; resolver tenant read-only completato in I4.3BF; provider re-fetch primitive completato in I4.3BG; pure admission classifier completato in I4.3BH; row observation reader completato in I4.3BI (helper **NON WIRED**); composer refetch→normalize completato in I4.3BJ (helper **NON WIRED**); restano runtime wiring/orchestration del composer / runtime wiring del resolver BF / runtime wiring del reader BI / ownership check / classifier runtime invocation / admission branch execution / persistence/CAS / Snapshot H2 / wiring eventi / runtime/deploy remoto. Il prossimo slice I4.3B sarà deciso dal Supervisor **dopo** review di questo documentale + commit I4.3BJ-bis + sync mirror controllata + verifica cloud + push Git e verifica allineamento; **NON** avviare autonomamente ownership / persistence / CAS / wiring / Snapshot / H2 / deploy / apply remoto.
...
**I4.3B — subscription sync e tenant snapshot** (schema M2 + normalizzatore puro + resolver helper read-only + provider re-fetch primitive + pure admission classifier + row observation reader + composer refetch→normalize già in repo; runtime wiring/orchestration ancora da implementare in micro-task successivi)

Punti ancora da implementare (non trasformare in implementazione in questo consolidamento documentale):

- Runtime wiring/orchestration del composer I4.3BJ nel path subscription/webhook (helper composer già presente; **non** wired)
- Runtime wiring/orchestration del reader I4.3BI nel webhook (helper read-only già presente; **non** wired)
...
- Ownership check: confrontare `tenant_subscriptions.tenant_id` osservato (I4.3BI) vs tenant risolto da I4.3BF — mismatch fail-closed / no remap
- Conversione `processed_at` NULL/NOT NULL → `billing_event_processed` boolean per il classifier
- Classifier invocation runtime (classifier puro già in I4.3BH; ramo classificato non eseguito)
...
| Push `origin/main` di I4.3BJ | **Non** attestato in preflight I4.3BJ-bis (`main` ahead 1; HEAD `e87462c`; `origin/main` `cc36c06`); nessun `git fetch` |
| Sync mirror post I4.3BJ-bis | **Non ancora eseguita** (obbligatoria dopo il futuro commit di questo consolidamento; non inventare hash) |
...
- **Stato mirror corrente (post I4.3BI-D1):** allineato e verificato rispetto al consolidamento `cc36c06`. Git reale resta fonte canonica. Nessuna riconciliazione Drive → repository.
- `docs/stato-operativo.md` viene aggiornato da questo task (I4.3BJ-bis) e sarà sincronizzato **solo dopo** il relativo futuro commit (e conferma utente secondo workflow governance: dry-run → validazione → apply → byte/hash → verifica cloud). Il commit documentale corrente **non** è registrato come ultimo consolidato in questo stesso aggiornamento (hash futuro non inventato). Sync mirror I4.3BJ-bis **non** eseguita in questo task e **non** dichiarata già avvenuta. I4.3BJ applicativo (`e87462c`) **non** è dichiarato pushato.
