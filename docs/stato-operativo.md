...
|------|--------|
| Branch atteso | `main` |
| Commit applicativo di riferimento | `38e428001f681e6c1a7d6c73970bf62bd3ad4dbd` — *feat(billing): add subscription event admission classifier* — I4.3BH |
| Ultimo commit governance/documentale consolidato | `bb0f7757bf8489dbbddfc1f7f4d6799e7581ece6` — *docs(context): record Stripe subscription refetch primitive* — I4.3BG-bis |
| Verifica Git richiesta | All’inizio di ogni task: `git rev-parse HEAD`, `git status`, `git branch --show-current` |

Nota: l’HEAD reale deve essere verificato all’inizio di ogni task con `git rev-parse HEAD`. Il commit che aggiorna `stato-operativo.md` può essere successivo all’ultimo commit registrato nel documento; questo non costituisce automaticamente una divergenza. Branch, working tree e cronologia reale del repository prevalgono sempre sui valori storici riportati nel documento. Non riportare come HEAD stabile il futuro commit del task documentale corrente (I4.3BH-bis).
...
| Commit applicativo corrente | `38e4280` — *feat(billing): add subscription event admission classifier* (I4.3BH) |
| Commit applicativo precedente rilevante | `dc5cbcf` — *feat(billing): add Stripe subscription refetch primitive* (I4.3BG); `48782cd` — *feat(billing): add billing customer tenant resolver* (I4.3BF); `0e42dd5` — *feat(billing): add Stripe subscription normalizer* (I4.3BE); `8246000` — *feat(billing): add subscription sync concurrency schema* (I4.3BD); prima ancora `18a4bf9` (I4.3A, include fix review F1–F4); `1f633fcc` (I4.2) |
...
| **Debito operativo `processed_at=NULL`** | Oggi significa sia deferred intenzionale sia evento incompleto — da affrontare separatamente; non in I4.3BH-bis |
...
**I4.3BH:** **CHIUSO APPLICATIVAMENTE / COMMITTATO LOCALMENTE** (non ancora pushato). Commit `38e4280` — *feat(billing): add subscription event admission classifier*. Classificatore puro/deterministico admission W_sub: input Event.created + Event.id + `billing_event_processed` + row observation (`ROW_ABSENT` vs `ROW_PRESENT`+W_sub); **non** esegue il ramo. Distinzioni: ROW_ABSENT ≠ present NULL/NULL; half-null → `invalid_watermark`; newer/stale solo da Event.created; equal ts + ID distinti → candidate (nessun tie-break lessicografico su event.id); same Event + not processed → `partial_retry` (**non** completed); same Event + processed → `already_applied`; same Event ID + ts diverso → `inconsistent_same_event`. Zero DB/Supabase/Stripe/I/O/persistence/CAS/Snapshot/H2/wiring. Quality: `npm run lint` PASS; `npm run build` PASS; warning Vite chunk size preesistente/non bloccante; `deno check` PASS; `deno test` 18/18 PASS; review Supervisor PASS (whitespace `git diff --no-index --check` sui due file untracked); nessun I4.3BH-F1. Rischio minore non bloccante: `billing_event_processed` boolean non validato runtime — futuro wiring da `processed_at` NULL/NOT NULL. Allo stato Git locale di preflight I4.3BH-bis: `main` ahead di `origin/main` di 1 — **non** attestare push di I4.3BH. K2 complessivo = ancora **NON** implementato (classifier puro ≠ runtime admission/persistence/CAS/orchestration).
...
**I4.3B:** **NON COMPLETO** complessivamente. Schema M2 / W_sub / `billing_state_revision` / G1 = **completati** in I4.3BD. Normalizzatore provider puro = **completato** in I4.3BE. Tenant resolution helper read-only = **completato** in I4.3BF. Provider re-fetch primitive = **completato** in I4.3BG. Pure admission classifier W_sub = **completato** in I4.3BH. Restano **non** implementati: mapping/read runtime della row observation `tenant_subscriptions`; tenant resolution runtime/wiring; composition refetch→normalize; provider re-fetch K2 runtime/orchestration; persistence `tenant_subscriptions` (INSERT/conditional UPDATE/CAS/23505/CAS failure readback); runtime admission W_sub; gestione partial/incomplete downstream; Snapshot(S) V3 applicativa; H2 CAS V2 / TenantGuard; wiring `customer.subscription.created|updated|deleted`; valorizzazione `billing_events.tenant_id` nel path subscription; `processed_at` completion; runtime/deploy; apply remoto migration 007. Invoice fuori scope del primo I4.3B. Eventi `customer.subscription.*` restano differiti finché un task applicativo successivo non implementa esplicitamente i slice residui. K2 congelato resta: provider re-fetch + admission watermark W_sub + CAS/readback + nuovo provider re-fetch dopo CAS failure se l’Event resta candidato + retry locale limitato + fail/retry se contention non converge — I4.3BG implementa il primitive refetch; I4.3BH implementa **soltanto** il classificatore puro di admission; runtime observation / persistence / CAS / orchestration restano assenti.

**Filone corrente: I4.3B** — subscription sync e tenant snapshot (**NON COMPLETO**). Schema M2 completato in I4.3BD; normalizzatore puro completato in I4.3BE; resolver tenant read-only completato in I4.3BF; provider re-fetch primitive completato in I4.3BG; pure admission classifier completato in I4.3BH; restano row observation runtime mapping / runtime wiring del resolver / composition refetch→normalize / provider re-fetch K2 runtime/orchestration / persistence/CAS / runtime admission / Snapshot H2 / wiring eventi / runtime/deploy remoto. Il prossimo slice I4.3B sarà deciso dal Supervisor **dopo** review di questo documentale + commit I4.3BH-bis + sync mirror controllata + verifica cloud + push Git e verifica allineamento; **NON** avviare autonomamente row mapper / composition / persistence / CAS / wiring / Snapshot / deploy / apply remoto.
...
**I4.3B — subscription sync e tenant snapshot** (schema M2 + normalizzatore puro + resolver helper read-only + provider re-fetch primitive + pure admission classifier già in repo; runtime wiring/orchestration ancora da implementare in micro-task successivi)

Punti ancora da implementare (non trasformare in implementazione in questo consolidamento documentale):

- Mapping/read runtime della row observation `tenant_subscriptions` (presenza ROW_ABSENT vs ROW_PRESENT + W_sub; **non** collassare absent e present NULL/NULL)
...
- Provider re-fetch K2 runtime/orchestration (runtime admission usando il classifier I4.3BH + CAS/readback + nuovo re-fetch dopo CAS failure + bounded retry) — classifier puro ≠ K2 completo
...
- Gestione partial/incomplete downstream (guardrail F1: W_sub==Event AND `processed_at` NULL = partial, **non** completed; non riscrivere ciecamente subscription)
...
| Push `origin/main` di I4.3BH | **Non** attestato in preflight I4.3BH-bis (`main` ahead 1); nessun `git fetch` |
| Sync mirror post I4.3BH-bis | **Non ancora eseguita** (obbligatoria dopo il futuro commit di questo consolidamento; non inventare hash) |
| K2 complessivo / persistence / CAS / runtime admission / Snapshot / H2 / wiring | **Non** implementati (solo classificatore puro) |
| Apply remoto 007 / deploy / Stripe runtime / webhook wiring | **Non** eseguiti |
...
- **Stato mirror corrente (post I4.3BG-D1):** allineato e verificato rispetto al consolidamento `bb0f775`. Git reale resta fonte canonica. Nessuna riconciliazione Drive → repository.
- `docs/stato-operativo.md` viene aggiornato da questo task (I4.3BH-bis) e sarà sincronizzato **solo dopo** il relativo futuro commit (e conferma utente secondo workflow governance: dry-run → validazione → apply → byte/hash → verifica cloud). Il commit documentale corrente **non** è registrato come ultimo consolidato in questo stesso aggiornamento (hash futuro non inventato). Sync mirror I4.3BH-bis **non** eseguita in questo task e **non** dichiarata già avvenuta. I4.3BH applicativo (`38e4280`) **non** è dichiarato pushato.
