...
|------|--------|
| Branch atteso | `main` |
| Commit applicativo di riferimento | `dc5cbcfd6a9667364567efac957e9434535e85a2` — *feat(billing): add Stripe subscription refetch primitive* — I4.3BG |
| Ultimo commit governance/documentale consolidato | `a16eeb83d0e61f8b15ee1a8a66df94caa31372b4` — *docs(context): record billing customer tenant resolver* — I4.3BF-bis |
| Verifica Git richiesta | All’inizio di ogni task: `git rev-parse HEAD`, `git status`, `git branch --show-current` |

Nota: l’HEAD reale deve essere verificato all’inizio di ogni task con `git rev-parse HEAD`. Il commit che aggiorna `stato-operativo.md` può essere successivo all’ultimo commit registrato nel documento; questo non costituisce automaticamente una divergenza. Branch, working tree e cronologia reale del repository prevalgono sempre sui valori storici riportati nel documento. Non riportare come HEAD stabile il futuro commit del task documentale corrente (I4.3BG-bis).
...
| Commit applicativo corrente | `dc5cbcf` — *feat(billing): add Stripe subscription refetch primitive* (I4.3BG) |
| Commit applicativo precedente rilevante | `48782cd` — *feat(billing): add billing customer tenant resolver* (I4.3BF); `0e42dd5` — *feat(billing): add Stripe subscription normalizer* (I4.3BE); `8246000` — *feat(billing): add subscription sync concurrency schema* (I4.3BD); prima ancora `18a4bf9` (I4.3A, include fix review F1–F4); `1f633fcc` (I4.2) |
...
| **I4.3BG** | **CHIUSO APPLICATIVAMENTE / COMMITTATO LOCALMENTE** — provider re-fetch primitive (`dc5cbcf`); 12/12 Deno PASS; non ancora pushato; consolidamento documentale = questo task I4.3BG-bis (sync mirror **non** ancora eseguita). K2 complessivo ancora **NON** implementato |
| **I4.3B (filone corrente)** | Sync `customer.subscription.*` → `tenant_subscriptions` + snapshot `tenants` — **NON COMPLETO** complessivamente. Schema M2 / W_sub / `billing_state_revision` / trigger G1 = **completati** in I4.3BD. Normalizzatore provider puro = **completato** in I4.3BE. Tenant resolution helper read-only = **completato** in I4.3BF. Provider re-fetch primitive = **completato** in I4.3BG. Restano **non** implementati: tenant resolution runtime/wiring; composition refetch→normalize; provider re-fetch K2 runtime/orchestration (admission W_sub + CAS/readback + retry); persistence `tenant_subscriptions` / admission W_sub; gestione partial/incomplete; Snapshot(S) V3 applicativa; H2 CAS V2 / TenantGuard; wiring `customer.subscription.created|updated|deleted`; valorizzazione `billing_events.tenant_id` nel path subscription; `processed_at` completion; runtime/deploy; apply remoto migration 007. Policy Snapshot V3 **congelate** ma **non** coded nel runtime. Invoice fuori scope del primo I4.3B. Eventi `customer.subscription.*` restano differiti (`processed_at` nullo) finché un task applicativo successivo non implementa esplicitamente i slice residui |
| **Sync subscription** | Eventi `customer.subscription.*` / invoice: **solo persistiti** in `billing_events` con `processed_at` nullo; **non** risolvono `tenant_id` a runtime; **non** eseguono handler business; **non** aggiornano `tenant_subscriptions` né lo snapshot su `tenants`; rispondono HTTP 200 deferred. Architettura anti-stale/snapshot + policy prodotto progettate (K2+R-A+Snapshot V3+H2 V2); schema M2 presente in locale; normalizzatore puro presente; resolver helper read-only presente; provider re-fetch primitive presente; **non** wired a runtime |
...
| **Debito operativo `processed_at=NULL`** | Oggi significa sia deferred intenzionale sia evento incompleto — da affrontare separatamente; non in I4.3BG-bis |
...
62. **I4.3BG** — provider-authoritative Stripe Subscription refetch primitive (`refetchStripeSubscription.ts` + test), consolidato in `dc5cbcf` — *feat(billing): add Stripe subscription refetch primitive*; DI strutturale; zero fallback/cache; zero wiring/W_sub/CAS; non ancora pushato
63. **I4.3BG-bis** — consolidamento documentale refetch primitive (questo task; commit da registrare al prossimo aggiornamento significativo; sync mirror obbligatoria dopo il futuro commit)
...
**I4.3B:** **NON COMPLETO** complessivamente. Schema M2 / W_sub / `billing_state_revision` / G1 = **completati** in I4.3BD. Normalizzatore provider puro = **completato** in I4.3BE. Tenant resolution helper read-only = **completato** in I4.3BF. Provider re-fetch primitive = **completato** in I4.3BG. Restano **non** implementati: tenant resolution runtime/wiring; composition refetch→normalize; provider re-fetch K2 runtime/orchestration; persistence `tenant_subscriptions` / admission W_sub; gestione partial/incomplete; Snapshot(S) V3 applicativa; H2 CAS V2 / TenantGuard; wiring `customer.subscription.created|updated|deleted`; valorizzazione `billing_events.tenant_id` nel path subscription; `processed_at` completion; runtime/deploy; apply remoto migration 007. Invoice fuori scope del primo I4.3B. Eventi `customer.subscription.*` restano differiti finché un task applicativo successivo non implementa esplicitamente i slice residui. K2 congelato resta: provider re-fetch + admission watermark W_sub + CAS/readback + nuovo provider re-fetch dopo CAS failure se l’Event resta candidato + retry locale limitato + fail/retry se contention non converge — I4.3BG implementa **soltanto** il primo elemento riutilizzabile.

**Filone corrente: I4.3B** — subscription sync e tenant snapshot (**NON COMPLETO**). Schema M2 completato in I4.3BD; normalizzatore puro completato in I4.3BE; resolver tenant read-only completato in I4.3BF; provider re-fetch primitive completato in I4.3BG; restano runtime wiring del resolver / composition refetch→normalize / provider re-fetch K2 runtime/orchestration / persistence / admission W_sub / Snapshot H2 / wiring eventi / runtime/deploy remoto. Il prossimo slice I4.3B sarà deciso dal Supervisor **dopo** review di questo documentale + commit I4.3BG-bis + sync mirror controllata + verifica cloud + push Git e verifica allineamento; **NON** avviare autonomamente K2 / persistence / wiring / Snapshot / deploy / apply remoto.
...
**I4.3B — subscription sync e tenant snapshot** (schema M2 + normalizzatore puro + resolver helper read-only + provider re-fetch primitive già in repo; runtime wiring/orchestration ancora da implementare in micro-task successivi)
...
- Provider re-fetch K2 runtime/orchestration (admission W_sub + CAS/readback + nuovo re-fetch dopo CAS failure + bounded retry) — primitive refetch ≠ K2 completo
...
**Già implementato nel filone (non ripetere come assente):** schema M2/W_sub/revision/G1 (I4.3BD); normalizzatore puro Stripe Subscription con NP-A / NormalizationFailClosed / mono-item / metadata canonicale (I4.3BE); resolver tenant read-only fail-closed su `tenant_billing_customers` (I4.3BF); provider-authoritative Stripe Subscription refetch primitive `refetchStripeSubscription` (I4.3BG) — **non** elencare più il helper/primitive come totalmente assente; resta assente il runtime/orchestration K2.
...
| Push `origin/main` di I4.3BG | **Non** attestato in preflight I4.3BG-bis (`main` ahead 1); nessun `git fetch` |
| Sync mirror post I4.3BG-bis | **Non ancora eseguita** (obbligatoria dopo il futuro commit di questo consolidamento; non inventare hash) |
| K2 complessivo / CAS / W_sub runtime | **Non** implementati (solo primitive) |
| Apply remoto 007 / deploy / Stripe runtime / webhook wiring | **Non** eseguiti |
...
- **Stato mirror corrente (post I4.3BF-D1):** allineato e verificato rispetto al consolidamento `a16eeb8`. Git reale resta fonte canonica. Nessuna riconciliazione Drive → repository.
- `docs/stato-operativo.md` viene aggiornato da questo task (I4.3BG-bis) e sarà sincronizzato **solo dopo** il relativo futuro commit (e conferma utente secondo workflow governance: dry-run → validazione → apply → byte/hash → verifica cloud). Il commit documentale corrente **non** è registrato come ultimo consolidato in questo stesso aggiornamento (hash futuro non inventato). Sync mirror I4.3BG-bis **non** eseguita in questo task e **non** dichiarata già avvenuta. I4.3BG applicativo (`dc5cbcf`) **non** è dichiarato pushato.
