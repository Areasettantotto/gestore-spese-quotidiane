...
|------|--------|
| Branch atteso | `main` |
| Commit applicativo di riferimento | `0e42dd5027615beee3efc3ff8a9005bca3b7b4e7` — *feat(billing): add Stripe subscription normalizer* — I4.3BE |
| Ultimo commit governance/documentale consolidato | `aa2b1e72a8c0d0b4d80825985d8f2d8e8963830a` — *docs(context): record subscription concurrency schema* — I4.3BD-bis |
| Verifica Git richiesta | All’inizio di ogni task: `git rev-parse HEAD`, `git status`, `git branch --show-current` |

Nota: l’HEAD reale deve essere verificato all’inizio di ogni task con `git rev-parse HEAD`. Il commit che aggiorna `stato-operativo.md` può essere successivo all’ultimo commit registrato nel documento; questo non costituisce automaticamente una divergenza. Branch, working tree e cronologia reale del repository prevalgono sempre sui valori storici riportati nel documento. Non riportare come HEAD stabile il futuro commit del task documentale corrente (I4.3BE-bis).
...
- Mirror: Git canonico, Drive = consultazione. Sync post-commit I4.3BD-bis (`aa2b1e7`) **completata e verificata** (dry-run PASS; apply PASS; byte/hash 15/15 PASS; cloud verificato dal Supervisor; nessuna sync inversa; nessun --delete). Sync post-commit I4.3BE-bis **non ancora eseguita** (obbligatoria dopo il futuro commit di questo consolidamento; non inventare hash). Commit/push restano dell’utente. Config locale non versionata (AI_CONTEXT_MIRROR_DIR o .git/ai-context-mirror-path). Prima sync controllata completata su 15 file. Propagazione cloud = Google Drive Desktop. ChatGPT rilegge il mirror a ogni ripresa.
- ANCORA MANCANTE: sync subscription → tenant_subscriptions + snapshot tenants (I4.3B, **NON IMPLEMENTATO** / **NON COMPLETO** complessivamente; schema M2/W_sub/revision/G1 **implementati** in I4.3BD e testati in locale; normalizzatore provider puro **implementato** in I4.3BE; restano assenti tenant resolution / provider refetch K2 runtime / persistence `tenant_subscriptions` / admission W_sub / gestione partial/incomplete / Snapshot(S) V3 / H2 CAS V2 / TenantGuard / webhook wiring `customer.subscription.*` / `processed_at` completion / runtime-deploy / apply remoto migration 007; policy prodotto D3/D4/D5/D7/unpaid **congelate** in I4.3BC); invoice fuori scope I4.3B; billing portal; UI checkout reale; feature gating piani; tenant switcher/inviti. Drift storico di production-readiness.md e billing-data-model.md ancora presente.

Prossimo filone: I4.3B — subscription sync e tenant snapshot (**NON IMPLEMENTATO** / **NON COMPLETO** complessivamente). Schema M2 completato in I4.3BD; normalizzatore puro completato in I4.3BE; restano tenant resolver/refetch K2/persistence/admission W_sub/Snapshot H2/wiring/runtime/deploy remoto. Il prossimo slice I4.3B sarà definito dal Supervisor dopo commit di questo documentale + sync mirror + verifica cloud; **NON** avviare autonomamente persistence/wiring/Snapshot/deploy. I4.3A-T1 = CHIUSO COMPLESSIVAMENTE. I4.3A-D1 concluso. I4.3BA / I4.3BB / I4.3BB-R / F1–F3 / I4.3BC / F1–F3 / I4.3BD / I4.3BD-T1 / I4.3BD-bis / I4.3BE / I4.3BE-F1 / I4.3BE-T1 chiusi.
...
| Commit applicativo corrente | `0e42dd5` — *feat(billing): add Stripe subscription normalizer* (I4.3BE) |
| Commit applicativo precedente rilevante | `8246000` — *feat(billing): add subscription sync concurrency schema* (I4.3BD); prima ancora `18a4bf9` (I4.3A, include fix review F1–F4); `1f633fcc` (I4.2) |
...
| **I4.3BE / I4.3BE-F1 / I4.3BE-T1** | **CHIUSI** — normalizzatore puro Stripe Subscription (`0e42dd5`); F1 chiuso; T1 re-run `deno check` + `deno test` 28/28 PASS. Consolidamento documentale = questo task I4.3BE-bis (sync mirror **non** ancora eseguita) |
| **I4.3B (filone successivo)** | Sync `customer.subscription.*` → `tenant_subscriptions` + snapshot `tenants` — **NON IMPLEMENTATO** / **NON COMPLETO** complessivamente. Schema M2 / W_sub / `billing_state_revision` / trigger G1 = **completati** in I4.3BD. Normalizzatore provider puro = **completato** in I4.3BE. Restano **non** implementati: tenant resolution; provider refetch K2 runtime; persistence `tenant_subscriptions` / admission W_sub; gestione partial/incomplete; Snapshot(S) V3 applicativa; H2 CAS V2 / TenantGuard; wiring `customer.subscription.created|updated|deleted`; `processed_at` completion; runtime/deploy; apply remoto migration 007. Policy Snapshot V3 **congelate** ma **non** coded nel runtime. Invoice fuori scope del primo I4.3B. Eventi `customer.subscription.*` restano differiti (`processed_at` nullo) finché un task applicativo successivo non implementa esplicitamente i slice residui |
| **Sync subscription** | Eventi `customer.subscription.*` / invoice: **solo persistiti** in `billing_events` con `processed_at` nullo; **non** risolvono `tenant_id`; **non** eseguono handler business; **non** aggiornano `tenant_subscriptions` né lo snapshot su `tenants`; rispondono HTTP 200 deferred. Architettura anti-stale/snapshot + policy prodotto progettate (K2+R-A+Snapshot V3+H2 V2); schema M2 presente in locale; normalizzatore puro presente; **non** wired a runtime |
...
| **Debito operativo `processed_at=NULL`** | Oggi significa sia deferred intenzionale sia evento incompleto — da affrontare separatamente; non in I4.3BE-bis |
...
57. **I4.3BE-bis** — consolidamento documentale normalizzatore (questo task; commit da registrare al prossimo aggiornamento significativo; sync mirror obbligatoria dopo il futuro commit)
...
**I4.3B:** **NON IMPLEMENTATO** / **NON COMPLETO** complessivamente. Schema M2 / W_sub / `billing_state_revision` / G1 = **completati** in I4.3BD. Normalizzatore provider puro = **completato** in I4.3BE. Restano **non** implementati: tenant resolution; provider refetch K2 runtime; persistence `tenant_subscriptions` / admission W_sub; gestione partial/incomplete; Snapshot(S) V3 applicativa; H2 CAS V2 / TenantGuard; wiring `customer.subscription.created|updated|deleted`; `processed_at` completion; runtime/deploy; apply remoto migration 007. Invoice fuori scope del primo I4.3B. Eventi `customer.subscription.*` restano differiti finché un task applicativo successivo non implementa esplicitamente i slice residui.

**Prossimo filone: I4.3B** — subscription sync e tenant snapshot (**NON COMPLETO**). Schema M2 completato in I4.3BD; normalizzatore puro completato in I4.3BE; restano tenant resolver/refetch K2/persistence/admission W_sub/Snapshot H2/wiring/runtime/deploy remoto. Il prossimo slice I4.3B sarà definito dal Supervisor **dopo** commit di questo documentale + sync mirror controllata + verifica cloud; **NON** avviare autonomamente persistence / wiring / Snapshot / deploy / apply remoto.
...
**I4.3B — subscription sync e tenant snapshot** (schema M2 + normalizzatore puro già in repo; runtime wiring ancora da implementare in micro-task successivi)

Punti ancora da implementare (non trasformare in implementazione in questo consolidamento documentale):

- Tenant resolution da `subscription.customer` → `tenant_billing_customers`
...
**Già implementato nel filone (non ripetere come assente):** schema M2/W_sub/revision/G1 (I4.3BD); normalizzatore puro Stripe Subscription con NP-A / NormalizationFailClosed / mono-item / metadata canonicale (I4.3BE).
...
| Push `origin/main` di I4.3BE | **Non** attestato in preflight I4.3BE-bis (`main` ahead 1); nessun `git fetch` |
| Sync mirror post I4.3BE-bis | **Non ancora eseguita** (obbligatoria dopo il futuro commit di questo consolidamento) |
...
- **Stato mirror corrente (post I4.3BD-bis):** allineato e verificato rispetto al consolidamento `aa2b1e7`. Git reale resta fonte canonica. Nessuna riconciliazione Drive → repository.
- `docs/stato-operativo.md` viene aggiornato da questo task (I4.3BE-bis) e sarà sincronizzato **solo dopo** il relativo futuro commit (e conferma utente secondo workflow governance: dry-run → validazione → apply → byte/hash). Il commit documentale corrente **non** è registrato come ultimo consolidato in questo stesso aggiornamento (hash futuro non inventato). Sync mirror I4.3BE-bis **non** eseguita in questo task e **non** dichiarata già avvenuta.
